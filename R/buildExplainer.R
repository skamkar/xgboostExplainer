...
#' @param treeIndx an integer vector of tree indices that should be parsed.
...
#' explainer = buildExplainer(xgb.model,xgb.train.data, type="binary", base_score = 0.5, treeIndx = 0:4)
...
buildExplainer = function(xgb.model, trainingData, type = "binary", base_score = 0.5, treeIndx = NULL){

  col_names = attr(trainingData, ".Dimnames")[[2]]
  cat('\nCreating the trees of the xgboost model...')
  trees = xgb.model.dt.tree(col_names, model = xgb.model, trees = treeIndx)
